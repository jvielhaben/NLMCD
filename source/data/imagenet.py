...
# Sure, here's a balanced list of 50 classes from the provided dataset, categorized into mammals, objects, and vehicles
CHATGPT50_CLASSES = [
    "goldfish",
    "great white shark",
    "tiger shark",
...
RESNET_FEATUREMAP_WIDTH = {1: 56, 2: 28, 3: 14, 4: 7}
...
class Hook:
    ...
    def __call__(self, module, module_in, module_out):
        self.hooked = module_out

    def clear(self):
        self.hooked = None


def get_model_architecture_in(model_str):
    if "VisionTransformer" in model_str:
        return "vit"
    elif "ResNet" in model_str:
        return "resnet"


def get_model_encoder_in(
    model, feature_layer, post_vcl_insertion=False, pre_layernorm=False, mlp=False
):
    architecture = get_model_architecture_in(str(model))
    if architecture == "vit":
        if post_vcl_insertion:
            return model.blocks
        else:
            if mlp:
                return model.blocks[feature_layer - 1].mlp
            elif pre_layernorm:
                return model.blocks[feature_layer - 1]
            else:
                # extract after first layer norm of next block
                if feature_layer < len(model.blocks):
                    return model.blocks[feature_layer].norm1  # 12 blocks
                elif feature_layer == len(model.blocks):
                    return model.norm
    elif architecture == "resnet":
        # return model.stages[feature_layer-1]   # 4 blocks
        return getattr(model, f"layer{feature_layer}")  # 4 blocks


def pool_tokens(input_tensor, avg_pool):
    # Ensure the input tensor is of shape (1, n_token, d)
    # assert input_tensor.dim() == 3 and input_tensor.size(0) == 1

    # Get the size of n_token and d
    d = input_tensor.size(2)

    # Reshape to (1, d, 14, 14) for AvgPool2d
    input_tensor = input_tensor.permute(0, 2, 1).view(1, d, 14, 14)

    # Apply the pooling operation
    input_tensor = avg_pool(input_tensor)

    # The output size will be (1, d, 7, 7), then reshape to (1, 49, d)
    input_tensor = input_tensor.view(1, d, -1).permute(0, 2, 1)
    return input_tensor


class ThroughModel_in(torch.nn.Module):
    def __init__(
        self,
        model,
        feature_layer,
        ...
        if self.pool_token:
            # Create an AvgPool2d layer with a kernel size of 2 and stride of 2
            self.avg_pool = torch.nn.AvgPool2d(kernel_size=kernel_size, stride=1)

        if feature_layer > 0:
            self.hook = Hook()
            get_model_encoder_in(
                self.model, feature_layer, pre_layernorm=pre_layernorm, mlp=mlp
    ...
    def forward(self, x):
        ...
        if self.hook is not None:
            out = self.hook.hooked
        if self.remove_cls:
            out = out[:, 1:]  # tensor is shape (1,n_token,d) for ViT
            if self.pool_token:
                out = pool_tokens(out, self.avg_pool)
        elif self.remove_sequence:
            out = out[
                :,
                [
                    0,
...
def get_trafos_in(cfg, cuda=True):
    # dimension transformer
    # depends on architecture
    model = timm.create_model(cfg.params.representation_model_ckpt, pretrained=True)
    model = model.eval()
    # pass through model
    ...
    if cfg.params.feature_layer > 0:
        ...
        # for spatial unravelling
        if architecture == "vit":
            dimension_trafo_load, dimension_trafo_collate = "", "0,1,2->01,2"  # ViT
        elif architecture == "resnet":
            dimension_trafo_load, dimension_trafo_collate = "", "0,1,2,3->023,1"  # CNN
    else:
...
def get_labels_in(y, X_shape, dt_collate_label):
    y_patch = dimension_collator_label(y, X_shape, dt_collate_label, numpy=True)
    return y_patch


def stratified_subsets(
    indices, labels, subset_index, subsample_ratio=0.1, random_seed=42
):
    """
    Dissect the dataset into label-stratified subsets using a subsample ratio.

    Parameters:
    indices (np.array or torch.Tensor): Array of indices of the points in the dataset.
    labels (np.array or torch.Tensor): Array of corresponding labels.
    subsample_ratio (float): Ratio to determine the number of subsets. For example, 0.1 will create 10 subsets.
    random_seed (int): Seed for reproducibility.

    Returns:
    list of np.array: List of arrays where each array contains the indices of a subset.
    """
    ...
    num_subsets = int(1 / subsample_ratio)
    ...
    for label in unique_labels:
        ...
        # Determine the number of indices to take from this label for each subset
        num_label_indices_per_subset = min(
            subset_size // len(unique_labels), len(label_indices) // num_subsets
        )

        start = 0
        for i in range(num_subsets):
            end = start + num_label_indices_per_subset
            subsets[i].extend(label_indices[start:end])
            start = end

    # Flatten and shuffle each subset to ensure equal size
    for i in range(num_subsets):
        np.random.shuffle(subsets[i])
        subsets[i] = np.array(subsets[i][:subset_size])

    return np.array(subsets[subset_index])


class SubsetIDX(Subset):
    def __init__(self, dataset, indices, return_label=True):
        super(SubsetIDX, self).__init__(dataset, indices)
        self.return_label = return_label

    def __getitem__(self, idx):
        sample, label = super(SubsetIDX, self).__getitem__(idx)
        if not self.return_label:
            label = self.indices[idx]  # Get the original index
        return sample, label  # Return sample and its original index


def create_dataset(cfg, return_label, cuda, train, indices_subsample=None):
    # print('Creating dataset.')
    transforms, dimension_trafo_collate = get_trafos_in(cfg, cuda=cuda)

    root = cfg.params.root
    # dataset = torchvision.datasets.ImageNet(root, split='train' if train else 'val', transform=transforms)
    dataset = CustomImageNet(
        root, split="train" if train else "val", transform=transforms
    )
    ...
    return dataset, dimension_trafo_collate
...
def imagenet_loader(
    ...
    dataset, dimension_trafo_collate = create_dataset(
        cfg, return_label, cuda, train, indices_subsample=indices_subsample
    )
